...
- 단계 제목 수정
- 단계 설명 수정
- ChatGPT, Gemini, Claude로 단계 제목과 설명 자동 작성
- 단계 순서 변경
- 필요 없는 단계 삭제
- 빨간 마커 위치 보정
- PDF 파일 저장
...
편집 화면 오른쪽의 `AI 문구 작성` 영역에서 생성형 AI 모델을 선택하고 API 키를 입력한 뒤 `문구 작성`을 누르면 선택한 단계의 제목과 설명이 자동으로 채워집니다.
...
API 키 입력칸은 비밀번호 입력란처럼 표시되어 입력한 키가 화면에 그대로 보이지 않습니다. API 키는 자동 저장하지 않으며, 현재 편집 화면을 열어 둔 동안에만 사용됩니다.

`문구 작성`을 누르면 현재 선택한 단계의 스크린샷, 클릭 마커, 페이지 제목, URL, 클릭 대상 텍스트가 선택한 AI 제공자 API로 전송됩니다. 민감한 정보가 보이는 단계에서는 이 기능을 사용하지 않는 것이 좋습니다.

<img src="docs/images/usage-04-ai-copy.jpg" alt="ClickGuide Local 편집 화면에서 AI 모델을 선택하고 API 키를 입력한 뒤 문구 작성 버튼을 누르는 화면" width="900">
...
- 선택한 AI 제공자의 API 키가 올바른지 확인합니다.
- API 키에 사용 한도나 결제 제한이 걸려 있지 않은지 확인합니다.
- 회사나 학교 네트워크에서 외부 AI API 호출을 막고 있지 않은지 확인합니다.
- 민감한 화면에서는 AI 문구 작성을 사용하지 않고 제목과 설명을 직접 입력합니다.
...
기본 녹화와 PDF 저장 기능은 기록한 내용을 외부 서버로 보내지 않습니다. 다만 `AI 문구 작성`을 누르면 선택한 단계의 스크린샷과 단계 정보가 사용자가 선택한 AI 제공자 API로 전송됩니다. API 키는 화면에 그대로 보이지 않도록 입력되며 자동 저장하지 않습니다.
...
- 클릭 시점 스크린샷 저장
- 단계별 설명 편집
- ChatGPT, Gemini, Claude를 이용한 단계 문구 자동 작성
- 단계 순서 변경
- 불필요한 단계 삭제
- 클릭 마커 위치 보정
- PDF 가이드 저장
...
src/editor/aiCopy.ts               AI 단계 문구 생성 처리
